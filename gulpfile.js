...
gulp.task('ts-reload', gulp.series('compile-ts:dev', function (callback) {
	$.browserSync.reload();
	callback();
}));
...
gulp.task('dev', gulp.series(['compile-ts:dev', 'bundle-sass:dev', 'copy-images:dev'], function () {
	$.browserSync.init({
		server: {
			baseDir: './public/',
...
gulp.task('release', gulp.series(['bundle-sass:release', 'compile-ts:release', 'copy-images:release'], function (done) {
